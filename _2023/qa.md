...
- `/sys` - Contains information and configuration for the system (covered in the [first lecture]({{'/2023/course-shell/' | relative_url}}))
...
The easiest tool that is quite useful for profiling purposes is [print timing]({{'/2023/debugging-profiling/#timing' | relative_url}}).
...
More generally a couple of more unconventional data wrangling tools are vim and Python. For some complex and multi-line transformations, vim macros can be a quite invaluable tool to use. You can just record a series of actions and repeat them as many times as you want, for instance in the editors [lecture notes]({{'/2023/editors/#macros' | relative_url}}) (and last year's [video](/2019/editors/)) there is an example of converting an XML-formatted file into JSON just using vim macros.
